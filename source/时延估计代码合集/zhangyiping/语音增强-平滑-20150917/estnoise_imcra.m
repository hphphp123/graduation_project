...
% eps_cap=alpha*(GH1.^2).*gamma_old+(1-alpha)*max(gamma-1,0); %有音时先验信噪比个估算公式（3-63）
...
gamma = min(S./noise_cap,40);      %后验信噪比
eps_cap=alpha*(GH1.^2).*gamma_old + (1-alpha)*max(gamma-1,0);   %先验信噪比，式3-63 
eps_cap=max(eps_min,eps_cap);  % 控制先验信噪比大于 -25 dB？？
v=gamma.*eps_cap./(1+eps_cap);
...
L=length(eps_cap);
for i=1:L
    if eps_cap(i) == eps_min;
       nn = nn+1;
       ESmin = ESmin + Smin(i);
    end
end
% ESmin = ESmin /nn;
% Bmin = noise_cap/ESmin;
% Bmin = min(Bmin,0.5);
% Bmin = max(Bmin,1.66);
...
p=zeros(len,1);                           %仅计算无音概率不为0部分，即可能有音部分
if (~isempty(index1))
...
noise_ps(1,1) = noise_cap(1,1);
for f=2:len-1
    noise_ps(f,1)=sum(b.*[noise_cap((f-1),1);noise_cap(f,1);noise_cap((f+1),1)]); 
end
noise_ps(len,1)=noise_cap(len,1);
